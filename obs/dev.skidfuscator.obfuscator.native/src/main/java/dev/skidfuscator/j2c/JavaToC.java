package dev.skidfuscator.j2c;

import dev.skidfuscator.j2c.annotations.AntidebugSelf;
import dev.skidfuscator.j2c.support.ClassMethodPair;
import dev.skidfuscator.j2c.support.ExtractedBytecode;
import dev.skidfuscator.j2c.visitors.ClassAnnotationExplorer;
import dev.skidfuscator.j2c.visitors.ClassBytecodeExtractor;
import dev.skidfuscator.j2c.visitors.ClassCodeEliminator;
...
public class JavaToC {
    ...
    public void startParsing(File outCSource) throws IOException {
        ...
        generator.addHeader(generator.getResources("temp.h"));
     ...
     */
    public void parseClass(final ClassNode input) throws IOException {
        if (!parsing) {
        ...
        input.node.accept(annotationExplorer);
        ...
        input.node.accept(bytecodeExtractor);
        ...
        final org.objectweb.asm.tree.ClassNode bew = new org.objectweb.asm.tree.ClassNode();
        codeEliminator = new ClassCodeEliminator(ASM_VERSION, methodsToProcess, libname.substring(3), bew);
        input.node.accept(codeEliminator);
        input.node = bew;
