...
import dev.skidfuscator.obfuscator.skidasm.expr.SkidDispatcherStaticInvocationExpr;
...
public class NativeDriverTransformer extends AbstractTransformer {
    public NativeDriverTransformer(Skidfuscator skidfuscator) {
                    ...
                    expr = new PredicateFlowGetter() {
                        ...
                        public Expr get(BasicBlock vertex) {
                            if (methodNode.isClinit())
                                return new SkidDispatcherStaticInvocationExpr(
                                        new Expr[]{
    ...
    void handle(final InitSkidTransformEvent event) {
        try {
            predicateCache = NativeTransformer.DEBUG
                    ? new SkidClassNode(
                    ClassHelper.create(NativeTransformer.CACHE).node,
                    skidfuscator
            )
                    : new SkidClassNode(
        ...
        final String name = this.getConfig().getName().equals("renamer")
                ? ClassRenamerTransformer.DICTIONARY.next()
    ...
    protected <T extends DefaultTransformerConfig> T createConfig() {
        return (T) new NativeDriverConfig(skidfuscator.getTsConfig(), MiscUtil.toCamelCase(name));
    }

    @Override
    public NativeDriverConfig getConfig() {
        return (NativeDriverConfig) super.getConfig();
    }
}
